...
struct BinTreeNode {
 ...
 struct BinTree {struct BinTreeNode *p;} ;
...
PBinTree  create_BTree( void ) {
    /* 创建完整的二叉树 */
    PBinTree pbtree = (PBinTree)malloc(sizeof(struct BinTree));
    if (pbtree != NULL)
        pbtree->p = createRest_BTree( );  /* 递归创建从根开始的二叉树 */
    return pbtree; 
}
...
int main(){
	...
	printf("二叉树先根输出序列为：\t");
    postOrder(tree->p);
	printf("二叉树节点个数为 Num=%d ",num_of_Btree(tree->p));
    putchar('\n');
    return 0;
}
